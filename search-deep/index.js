...
//ToDo: Deep search
//check all props of object, and filter by string, return  all objects where 
//you can find that string.
//minimum length of string = 2 

function checkFilterInObject(object, filter){
  let filterExist = false;
  for(let key in object){
    if(typeof object[key] !== 'object'){
      if(String(object[key]).includes(filter))
        filterExist = true;
    } else {
      if(checkFilterInObject(object[key], filter))
        filterExist = true;
    }
  }
  return filterExist;
...
function deepFilter(arr, filter){
    if(filter.length <= 2)
        return [];
  
    const filtratedArr = arr.filter(obj => {
      return checkFilterInObject(obj, filter)
    });

    return filtratedArr;
}

console.log(deepFilter(data, 'est'));